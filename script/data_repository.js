...
const repository = {
  ...
  uploadFood: async (formData, imageFile) => {
    return new Promise((resolve, reject) => {
      const image_name = repository.uploadImage(
        imageFile,
  ...
  deleteOrderFood: async (orderId, foodId) => {
    return new Promise((resolve, reject) => {
      pool.getConnection(async (err, connection) => {
        ...
        connection.query(
          `DELETE FROM orders_items where order_id = ? AND food_id = ?`,
          [orderId, foodId],
          (error, results) => {
            connection.release();
            if (error) {
  ...
  deleteFood: async (id) => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "UPDATE foods SET deleted_at = CURRENT_TIMESTAMP where id = ?",
          [id],
          (error, results) => {
            connection.release();
            if (error) {
  ...
  getFoodCateories: async () => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "SELECT * FROM foods_category ORDER BY sort ASC",
          (error, results) => {
            connection.release();
            if (error) {
              reject(error);
  ...
  getTradeNoById: async (id) => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "SELECT trade_no FROM table_orders where id = ?",
          [id],
          (error, results) => {
            connection.release();
            if (error) {
  ...
  getOrderByTradeNo: async (trade_no) => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "SELECT * FROM table_orders where trade_no = ?",
          [trade_no],
          (error, results) => {
            connection.release();
            if (error) {
  ...
  getPendingTableOrders: async () => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "SELECT * FROM table_orders where order_status = 1",
          (error, results) => {
            connection.release();
            if (error) {
              reject(error);
  ...
  addTableOrder: async (tableNum) => {
    return new Promise((resolve, reject) => {
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          "SELECT * FROM table_orders WHERE order_status = 1 AND table_number = ?",
          [tableNum],
          (error, results) => {
            if (error) {
              reject(error);
            ...
            if (results.length > 0) {
              ...
              connection.query(
                "INSERT INTO table_orders (trade_no, table_number) VALUES(?,?)",
                [repository.genearteTradeNo(), tableNum],
                (error, results) => {
                  connection.release();
                  if (error) {
                    reject(error);
  ...
  updateOrderFoods: async (orderId, data) => {
    return new Promise((resolve, reject) => {
      pool.getConnection(async (err, connection) => {
        ...
        var sqlStr =
          "INSERT INTO orders_items (order_id, food_id, quantity, unit_price, total_price) VALUES";
        var valuesSet = [];
        var values = [];
        data.forEach((item) => {
  ...
  confirmPaymentByCash: (order_id) => {
    return new Promise(async (resolve, reject) => {
      console.log("confirmPaymentByCash ");
      ...
      if (order && order_foods.length) {
        ...
        pool.getConnection((err, connection) => {
          ...
          connection.query(
            "UPDATE table_orders SET food_price = ?, service_fee = ?, trade_amt = ?, order_status = ?, payment_at = CURRENT_TIMESTAMP WHERE id = ?",
            [food_price, service_fee, trade_amt, order_status, order_id],
            (error, results) => {
              connection.release();
              if (error) {
                reject(error);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (order && order_foods.length) {
        ...
        pool.getConnection((err, connection) => {
          ...
          connection.query(
            "UPDATE table_orders SET food_price = ?, service_fee = ?, trade_amt = ? WHERE id = ?",
            [food_price, service_fee, trade_amt, order_id],
            (error, results) => {
              connection.release();
              if (error) {
                reject(error);
